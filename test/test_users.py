from io import BytesIO  # For files upload
from pathlib import Path # Reading the test image 
from unittest.mock import AsyncMock, patch # mocking email function
...
async def test_create_user_validation_error(client: AsyncClient):
    ...
    assert response.status_code == 422  # data format is wrong 
...
async def test_create_user_duplicate_email(client: AsyncClient):
    ...
    assert response.status_code == 400 # When the data is fine, but it breaks the rule we set for the app
...
async def test_create_user_success(client: AsyncClient):
    ...
    # Verifying the password is not in the response
...
async def test_upload_profile_picture(client: AsyncClient, mocked_aws):
    ...
    test_image_path = Path(__file__).parent / "test_image.jpg"   # __file__ refer to this file
    image_bytes = test_image_path.read_bytes()

    response = await client.patch(
        f"/api/users/{user['id']}/picture",
        files={"file": ("profile.jpg", BytesIO(image_bytes), "image/jpeg")},  # (file-name, file-content, file-type)
...
async def test_forgot_password_sends_email(client: AsyncClient):
    ...
    with patch(
        ...
        mock_send.assert_awaited_once() # we need to await async background task 
